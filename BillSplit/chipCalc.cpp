...
double chipBillTotal() {
	std::cout << '\n' << "2What is the total of the bill?" << '\n';
...
int chipSplits() {
	...
	for (int i{}; i < numPeople; ++i) {
		...
		std::cout << "How much did they chip in " << i + 1 << ": ";
		std::cin >> amountPeople[i].amount;

		// Clear the newline character from the input buffer
		std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

		amountPeople[i].remaining = split - total;
	}
...
void calculation() {
	...
	case 1:
		...
		oneAll();
	case 2-10: 
		std::cout << "If the number shown below is negative that means the person owes money." << '\n';
		std::cout << "If the number shown below is positive that means the person does not owe money." << '\n';
		for (int i{}; i < numPeople; ++i) {
			std::cout << amountPeople[i].name << ": " << '\n';
			std::cout << amountPeople[i].remaining << ": " << '\n';
		}
		std::cout << "The negative people can just paid the postive people however they want so that everybody = 0" << '\n';
	}
}